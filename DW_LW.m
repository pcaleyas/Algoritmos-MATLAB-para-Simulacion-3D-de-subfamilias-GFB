...
%USANDO EL METODO DE LW
%-----------------------------------------------------------------------------------------------

clear all

gamma = 3; % Parámetro de concentración, puede ser cualquier valor de R
...
Y=[cos(phi).*sx,sin(phi).*sx,X];
 ...
 % Graficar las muestras en una esfera (S_{2})
    ...
    x=Y(:,1); y=Y(:,2); z=Y(:,3); 
    plot3(x,y,z,'bo')
    title('Distribución DF-LW sobre la Esfera')
    hold on
    [x2,y2,z2] = sphere;
    obj= surf(x2,y2,z2);
